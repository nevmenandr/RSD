...
def find_all_tsv_files(root_dir):
    """Находит все файлы corpus.tsv в дереве директорий"""
    root_path = Path(root_dir).resolve()
    tsv_files = []
...
def calculate_dataset_stats(root_dir):
    ...
    # 3. Находим все TSV-файлы и извлекаем авторов и годы
    ...
    # Статистика по размеру корпусов
    # if corpus_folders:
        # folder_stats = []
        # for corpus_path in corpus_folders:
            # docs = 0
            ...
            # if docs > 0:
                # folder_stats.append((corpus_path.parent.name, docs, words))
        
        # folder_stats.sort(key=lambda x: x[1], reverse=True)
        # top_folders = ", ".join([f"{name} ({docs} док., {words:,} сл.)" for name, docs, words in folder_stats[:3]])
        # print(f"| Топ-3 корпуса по объему | {top_folders} |")
